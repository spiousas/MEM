...
Finalmente vamos a considerar un modelo de redes neuronales para preecir el cultivo. Por simplicidad el modelo contiene una sola capa intermedia y por medio de validación cruzada vamos a determinar cuántas neuronas tiene esta capa.
...
En la figura \ref{fig:DNN} observamos los resultados del Accuracy en función de la cantidad de neuronas de la capa intermedia y vemos que el máximo se alcanza en`r metricas_dnn$n[which.max(metricas_dnn$Accuracy)]` con un valor de `r round(max(metricas_dnn$Accuracy), digits = 3)`.

En la figura \ref{fig:DNN_esquema} podemos ver una representación del modelo que elegimos mediante validación cruzada.
