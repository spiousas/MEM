...
pred_glm_lasso <- function(p, lambda, folds) {
  ...
  for (i in 1:10) {
    fold <-folds$splits[[i]]
    data_train <- analysis(fold) %>% mutate(portadora = if_else(portadora == "1", 1, 0))
    data_val <- assessment(fold) %>% mutate(portadora = if_else(portadora == "1", 1, 0))
...
```{r}
testing_num <- testing %>%
  mutate(portadora = if_else(portadora == "1", 1, 0))
