...
- el número máximo muestras en
cada hoja, que puede tomar valores enteros mayores a 1 (`min_n`). 
...
A partir de esta matriz, el valor de F1 correspondiente es de `r round(f1_test, digits=3)`. A su vez, estimamos el error de clasificación y el resultado obtenido es xxx.
...
ROC <- roc(testing_num %>% pull(portadora), prob, plot = TRUE, legacy.axes = TRUE,
...
for (j in 1:length(ps)) {
  for (l in 1:length(lambdas)) {
    ...
    f1_clas_2vars <- f1_clas %>% add_row(tibble(p = ps[j], lambda = lambdas[l], f1 = metricas_p))
  }
}
```
...
Luego, ajustamos el modelo con los parámetros óptimos con toda la muestra de entrenamiento y evaluamos sobre la muestra de testeo el error de clasificación obteniendo un resultado de xxx. CONCLUIR SI AUMENTA MUCHO O POCO VEEER
