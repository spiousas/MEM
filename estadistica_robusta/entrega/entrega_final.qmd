...
Recordemos que $y_i = x_i + a$ con $a \in R$ y que $\textbf{y} = (y_1,..., y_n)$. Empecemos calculando $\bar{y}$ que seguramente lo vayamos a necesitar:
...
Y reemplacemos cada $y_i$ por $x_i + a$ y $\bar{y}$ por $\bar{x}+a$ (que sale del cálculo auxiliar):
...
#### Equivarianza por cambio de escala de $s$

Recordemos que $y_i = ax_i$ con $a \in R$ y que $\textbf{y} = (y_1,..., y_n)$. Empecemos calculando $\bar{\textbf{y}}$ que seguramente lo vayamos a necesitar:
...
Empecemos refrescando las propiedades de la mediana que vimos en el **ejercicio 1** (para más detalle ver la sección Anexo:Ejercicio 1)[^1].
...
\end{split}
$$
en ambos casos con $a \in \mathbb{R}$.

[^1]: Donde $\tilde{\textbf{x}}$ es la mediana de $\textbf{x}$.
...
#### Equivarianza por cambio de escala de la MAD

Recordemos que $y_i = ax_i$ con $a \in R$ y que $\textbf{y} = (y_1,..., y_n)$. En este caso queremos probar que $MAD(\textbf{y}) = |a| MAD(\textbf{x})$. Podemos retomar la $MAD(\textbf{y})$ definido anteriormente pero esta vez reemplazando cada $y_i$ por $ax_i$ y $\tilde{\textbf{y}}$ por $a\tilde{\textbf{x}}$ (que sale de las propiedades del ejercicio 1.b):
...
Luego, vamos a generar una matriz $x$, de la cual luego utilizaremos una sección en particular de acuerdo al n[^2].
...
Podemos ver las primeras tres y las últimas tres filas del tibble `grilla`, donde puede verse que para cada valor de `ns` hay `nrep` filas con el índice `rep`.

Lo siguiente que hacemos es calcular para cada fila el la media y la mediana para los primeras `ns` columnas de la fila `rep` de `x` (es un poco un trabalenguas, pero juro que tiene sentido). El código nos puede ayudar a entender:
...
En la figura @fig-eficiencias puede verse la representación gráfica de la eficiencia y el cociente de medias en función del tamaño de la muestra (n). En la misma podemos ver que efectivamente la eficiencia de la mediana converge a $2/\pi = 0.637$ mientras que el cociente entre las medias es $1$ para todo $n$. Esto último se debe a que tanto la media como la mediana son estimadores insesgados de $\mu$ entonces, sin importar el $n$ la esperanza de ambos (en este caso simulada como la media de `nrep` repeticiones) es $\mu$.
...
En la figura @fig-eficiencias_0 vemos que, efectivamente, el cociente entre las medidas tiene puntos raros debido a inestabilidades numéricas. También se observa que, para muestras más grandes donde es más esperable que la media y le mediana sean más parecidas hay menos puntos inestables. Sin embargo, como puede verse en el insert que magnifica la zona de `ns` $250$ a $1000$, el cociente de las medias está lejos de estabilizarse en $1$.
...
entonces el sumando de la suma nos queda:
...
recordando que $t = x_i/\sigma$. Entonces, lo siguiente que vamos a hacer es reescribirlo de forma tal de poder identificar $\rho()$ y $\delta$:
...
donde puede verse claramente que:
...
1. Ser una función no decreciente en $|t|$ (en particular, es una función par).
2. Que se cumpla que $\rho(0) = 0$.
3. Ser creciente (monótona) para $t > 0$ tal que $\rho(x) < \lim_{t \to+\infty} = \rho(\infty)$.
4. Que si $\rho$ es acotada, se supone $\rho(\infty) = 1$.
...
es decir, es una función par. Eso lo podemos ver también gráficamente. En la figura @fig-rhosym podemos ver a $\rho(t)$ en función de $t$ para $\nu=1$.
...
Ahora una fácil $\rho(0) = 0^2/(\nu + 0^2) = 0/\nu = 0$. Algo que también puede verse en la figura @fig-rhosym.
...
Como $\nu$ es una constante positiva y $t_a<t_b$ entonces $\nu/t_a > \nu/t_a$ y $1/\rho(t_a) - 1 > 1/\rho(t_b) - 1$, que es lo que queríamos demostrar. Esto también lo podemos ver graficamente. En la figura @fig-rhomonot se observa que, para valores de $t>0$, la $\rho(t)$ propuesta es monótona creciente.
...
Lo último que nos queda por probar es que $\rho(t)$ es acotada ($\rho(\infty) = 1$), algo que se ve fuertemente sugerido en la figura @fig-rhomonot. Para esto vamos a resolver el límite cuando $t \to \infty$ de $\rho(t)$:
