...
train <- titles_train_by_country |> 
  right_join(credits_train |> 
               filter(role == "DIRECTOR") |> 
               select(c("id", "name")) |>
               rename(director = "name"), by = "id")
splits <- initial_split(train, prop = 2/3, strata = type)
validation_data <- training(splits)
test_data <- testing(splits)
```

```{r}
model1 <- lmer(data = validation_data,
               imdb_score ~ release_year + type + runtime +
                 (1|director) + (1|production_countries))

#sum(test_data$imdb_score - predict(model1, newdata = test_data, allow.new.levels = TRUE))
```
