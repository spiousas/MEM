...
# Leemos los paquetes
pacman::p_load(tidyverse, ggdag, dagitty, here, modelsummary, patchwork, fastDummies, ggdist, lme4, mgcv, tidymodels)
```
...
credits_test <- read_csv(here("modelado_estadístico/TP2/data/credits_test.csv"),
                          show_col_types = FALSE, col_select = -1) 

titles_test <- read_csv(here("modelado_estadístico/TP2/data/titles_test.csv"),
                         show_col_types = FALSE, col_select = -1) |>
  mutate(genres = str_replace_all(genres, "\\[|\\]", ""),
         genres = str_replace_all(genres, "'", ""),
         production_countries = str_replace_all(production_countries, "\\[|\\]", ""),
...
Lo primero que vamos a hacer es explorar cómo se relaciona el género de una película con su calificación en IMDB. Para esto vamos a calcular el puntaje promedio y su variabilidad para cada género y visualizarlo de varias formas.
...
```{r, tab.cap="\\label{tab:ejemplo} Ejemplo de datos duplicados por género (primeros seis datos)."}
...
Como primera visualización vamos a proponer algo que no sólo nos deje ver una medida de resumen sino también la distribución del score para cada género. Es por eso que en la Figura \ref{fig:por_genero_ditrib} podemos ver un boxplot (en forma de línea) y una estimación no paramétrica de la densidad. Observamos que, en general, las distribuciones son unimodales y relativamente simétricas.

```{r, warning = FALSE, fig.align='center', echo = FALSE, fig.height=6, fig.cap ="\\label{fig:por_genero_ditrib}Boxplot y estimación no paramétrica de la densidad para cada género."}
titles_train_by_genre |> 
  group_by(genres) |>
  ggplot(aes(x = genres,
             y = imdb_score)) +
  stat_slab(alpha  = .5, fill = "steelblue", scale = .7) +
  stat_pointinterval(color = "darkgreen") +
  labs(x = NULL, y = "Rating en IMDB") +
...
Para seguir, en la Figura \ref{fig:por_genero} podemos ver una gráfica de barras de acuerdo al promedio de cada género y con la información de la cantidad de películas que son clasificadas como pertenecientes a ese género (como $n$). Puede verse que los tres géneros mejor calificados son Guerra, Historia y Documentales (en azul), mientras que géneros que socialmente suelen ser considerados "menores", o menos prestigiosos, como Terror y Comedia, se encuentran muy por debajo (en verde). 

```{r, by_genre, warning = FALSE, fig.align='center', echo = FALSE, fig.height=3.8, fig.cap ="\\label{fig:por_genero} Relación entre el género de las películas y la calificación promedio en IMDB. El $n$ indica la cantidad de títulos perteneciente a cada género. Las líneas grises que se extienden hacia la derecha indican el desvío estándar muestral."}
titles_train_by_genre |> 
  group_by(genres) |>
  summarise(m_rating = mean(imdb_score, na.rm = T),
            N = n(),
            sd_rating = sd(imdb_score, na.rm = T)) |>
  ...
  ggplot(aes(x = reorder(genres, m_rating),
             ...
             fill = top3)) +
  geom_col() +
  geom_errorbar(aes(ymin = m_rating, ymax = m_rating + sd_rating), 
...
Sin embargo, podemos ver que los géneros con más producciones son Drama y, justamente, Comedia. Entonces: ¿No es injusto que Western con $31$ producciones esté por arriba de Comedia con $1495$? Esta idea la vamos a desarrollar más en detalle cuando veamos las calificaciones por director.
...
```{r, by_director, warning = FALSE, echo = FALSE, warning = FALSE, fig.align='center', fig.height=3.8, fig.cap ="\\label{fig:director} Relación entre el director de las películas y la calificación promedio en IMDB. El $n$ indica la cantidad de títulos que pertenecen a cada director."}
num_on_top <- 10
...
```{r, by_director_weighted, echo = FALSE, warning = FALSE, fig.align='center', fig.height=6, fig.cap ="\\label{fig:director_pesado} Relación entre el director de la película y la calificación promedio en IMDB considerando el rating actualizado Rb ."}
rating_por_director <- titles_train |> 
  left_join(credits_train, by = "id") |>
  filter(role == "DIRECTOR") |>
...
```{r, echo = FALSE, warning = FALSE, fig.align='center', fig.height=3.7, fig.cap ="\\label{fig:pais_barra}Cantidad de producciones por país para los 20 países con más producciones en el dataset de entrenamiento."}
...
titles_train_by_country |>
  ...
  geom_text(aes(label = paste0(production_countries, " n=", N)), 
            color = "black", hjust = -0.1, size = 2.5) +
  labs(x = NULL, 
...
```{r}
fixed_countries <- lm(imdb_score ~ production_countries - 1,
...
En la figura se observan varias cosas. La más notoria es que a medida que disminuye la cantidad de títulos por país (hacia la izquierda) hay más diferencia entre las predicciones de ambos modelos. Esto ocurre porque el modelo de efectos fijos predice a cada país como el promedio de los títulos del mismo mientras que el de efectos mixtos considera una muestra de una distribución centrada en el promedio global (la línea punteada). De esta forma, cuanto más grande es el número de títulos de un país más cerca estará la estimación del modelo de efectos aleatorios al promedio del mismo (que a su vez es la estimación de efectos fijos), mientras que si $n$ es más chico, la estimación de efectos aleatorios estará más cercana al promedio global que al promedio de ese país.

\newpage
\blandscape

```{r, fig.height=8, fig.width=16, echo = FALSE, warning = FALSE, fig.align='center', fig.cap ="\\label{fig:pais_modelos}Predicciones de los modelos de efectos fijos y aleatorios para los países presentes en el dataset de entrenamiento (ordenados según cantidad de películas creciente de izquierda a derecha). La línea punteada vertical indica el promedio global de todas las calificaciones sin importar el país (promedio full pooleado)."}
...
titles_train_by_country |>
  ...
  geom_line(aes(group = production_countries), color = "black") +
  geom_point(size = 3) +
  annotate("segment", x = 30, y = 3.5, xend = 70, yend = 3.5, size = 1,
         arrow = arrow(type = "closed", length = unit(0.02, "npc"))) +
  annotate("text", x = 50, y = 3.5, size = 10, label = "n crece hacia la derecha", angle = 0,
           vjust = -0.5) +
...
pred_k <- tibble(release_year =
                   min(train_data$release_year):max(train_data$release_year))

# para cada K ajustamos el modelo y predecimos en una grilla
for (K in Ks) {
  fit <- gam(imdb_score ~ s(release_year, k = K , sp = 0, bs = 'cr'), 
             data = train_data)
  pred <- predict(fit, 
newdata = tibble(release_year = 
                   min(train_data$release_year):max(train_data$release_year)))
  var <- paste0("pred_k", K)
  pred_k <- pred_k %>%
...
La separación del conjunto de datos la haremos utilizando la función `initial_split` del paquete *{tidy_models}*. Separamos dos tercios de la muestra para entrenamiento y un tercio para el testeo. También definimos la función de pérdida con la que vamos a evaluar los modelos (`loss`) que corresponde al error cuadrático medio.
...
#### Modelo 1: Modelo lineal de efectos mixtos con Director como factor aleatorio.

El primer modelo que vamos a probar es un modelo lineal de efectos mixtos con el año de estreno (`release_year`) y la duración de la película (`runtime`) como factores fijos y el director (`director`) de la misma como factor aleatorio. El ajuste lo haremos utilizando la función `lmer` del paquete *{lme4}*. Para eso, previamente generamos nuestro conjunto `train` donde le agregamos a `train_data` una columna con los directores y seleccionamos las variables que se utilizan en este modelo.
...
```{r, warning=FALSE}
fit_m1 <- lmer(imdb_score ~ release_year + runtime + (1|director), 
               data = train)
```
...
pred <- predict(fit_m1, newdata = test, allow.new.levels = TRUE)

pred_director <-pred_director %>%
  mutate(pred=pred)

predicciones <- pred_director %>%
  group_by(id) %>%
...
#### Modelo 2: Modelo GAM con País de producción como factor aleatorio.

El segundo modelo que vamos a probar es un modelo ajustando splines utilizando la función `gam` del paquete *{mgcv}*. En este caso utilizaremos el año de estreno (`release_year`), la duración de la película (`runtime`) y el país de producción (`production_countries`). Los dos primeros son factores fijos mientras que el país de producción es un factor aleatorio. Antes de realizar el ajuste duplicamos las películas de `train_data` que tenían más de un país y así creamos el conjunto `train` seleccionando las columnas necesarias para este modelo.

```{r, echo=FALSE, warning=FALSE}
## MODELO 2 - spline con release_year, runtime y production_countries (random efects) ####
...
```{r, warning=FALSE}
fit_m2 <- gam(imdb_score ~ s(release_year, bs = 'cr') + s(runtime, bs = 'cr') + 
                s(production_countries, bs='re'), 
              data = train)
```
...
pred <- predict(fit_m2, newdata = test)
...
#### Modelo 3: Modelo GAM con Género como factor aleatorio.

Este modelo es muy similar al Modelo 2 con la diferencia de que en lugar de utilizar al país de producción de la película como factor aleatorio (modelado con splines cúbicas) como predictor, utilizamos al género de la película (`genres`). Al igual que en el Modelo 2, previo al ajuste duplicamos las películas que tuvieran más de un género y así creamos nuestro conjunto `train`.
...
```{r, warning=FALSE}
fit_m3 <- gam(imdb_score ~ s(release_year, bs = 'cr') + s(runtime, bs = 'cr') + 
                s(genres, bs='re'), 
              data = train)
```
...
# Predecimos
pred_genre<-tibble(id=test_data_by_genre$id)

pred<- predict(fit_m3, newdata = test)

pred_genre <-pred_genre %>%
  mutate(pred=pred)

predicciones <- pred_genre %>%
  group_by(id) %>%
  summarise(pred_prom = mean(pred, na.rm = TRUE)) %>%
  mutate(original_order=match(id,test_data$id)) %>%
  arrange(original_order)

error3 <- loss(predicciones$pred_prom,test_data$imdb_score)
```

#### Comparación entre modelos.

Finalmente, vamos a comparar el error cuadrático medio de los tres modelos a partir de las predicciones realizadas en el conjunto que separamos para testeo. Algo que vale la pena destacar es que en los tres modelos tenemos una variable categórica como factor aleatorio (director, país de producción o género) y podría ocurrir que a un título le correspondan varias categorías (por ejemplo, que sea una coproducción de varios países). En estos casos lo que hicimos para predecir fue duplicar los títulos que pertenezcan a varias categorías, realizar la predicción con cada una de ellas y luego promediamos todas las predicciones correspondientes a ese título.

En la tabla \ref{tab:comp_modelos} podemos ver que el modelo con menor error es el Modelo 3 (splines cúbicas y Género como factor aleatorio).  El error cuadrático medio es cercano a 1, lo que nos resulta razonable para la calificación de una película.

```{r, echo=FALSE, warning=FALSE, tab.cap="\\label{tab:comp_modelos} Comparación de los errores para los tres modelos evaluados."}
errores <- tibble(Modelo = c("Modelo lineal con director como FA",
                             "GAM con país como FA",
                             "GAM con género como FA"),
                  MSE = round(c(error1, error2, error3), 3))
...
Como vimos en la sección anterior, el Modelo 3 es el que tiene mejor desepeño con los datos de entrenamiento. Es por eso que lo utilizaremos para generar las predicciones de los títulos de los datos de testeo.

Primero ajustamos el modelo con todos los datos del conjunto de entrenamiento.

```{r, echo=FALSE, warning=FALSE}
# Data con generos duplicados
train_data_by_genre <- titles_train |>
  separate_rows(genres, sep = ", ") |>
  filter(genres != "") |>
  drop_na(imdb_score)

# Ajuste
train_completo <- train_data_by_genre %>%
  dplyr::select(c("imdb_score", "release_year", "runtime", "genres"))

train_completo$genres<- as.factor(train_completo$genres)
```

```{r, warning=FALSE}
fit_final <- gam(imdb_score ~ s(release_year, bs = 'cr') + s(runtime, bs = 'cr') + 
                   s(genres, bs='re'), 
                 data = train_completo)
```

Y ahora generamos las predicciones y las guardamos en un archivo `predicciones.csv`.

```{r, echo=FALSE, warning=FALSE}
# Data para testear
test_data <- titles_test

test_data_by_genre <- test_data |>
  separate_rows(genres, sep = ", ") 

test_completo <- test_data_by_genre %>%
  dplyr::select(c("release_year", "runtime", "genres"))

test_completo$genres <- as.factor(test_completo$genres)
```

```{r, warning=FALSE}
pred_genre <- tibble(id=test_data_by_genre$id)

pred <- predict(fit_final, newdata = test_completo)
...
predicciones <- pred_genre %>%
  group_by(id) %>%
  summarise(y_hat = mean(pred, na.rm = TRUE)) %>%
  mutate(original_order=match(id,test_data$id)) %>%
  arrange(original_order)

predicciones %>%
  select("y_hat") %>%
  write.csv(here("modelado_estadístico/TP2/predicciones/predicciones.csv"))
```
