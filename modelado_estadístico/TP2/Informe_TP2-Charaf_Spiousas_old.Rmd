...
Como primer visualización vamos a proponer algo que no sólo nos deje ver el promedio sino también la distribución para cada género. Es por eso que en la figura \ref{fig:por_genero_ditrib} podemos ver un boxplot (en forma de línea) y una estimación no paramétrica de la densidad. Vemos que, en general, las distribuciones son unimodales y relativamente simétricas.
...
```{r, warning = FALSE, fig.align='center', echo = FALSE, fig.height=6, fig.cap ="\\label{fig:por_genero_ditrib} Histograma y estimación no paramétrica de la densidad para cada género."}
...
```{r, by_genre, warning = FALSE, fig.align='center', echo = FALSE, fig.height=4, fig.cap ="\\label{fig:por_genero} Relación entre el género de las películas y la calificación promedio en IMDB. El $n$ indica la cantidad de títulos perteneciente a cada género. Las líneas grises que se extienden hacia la derecha indican el desvío estándar muestral."}
titles_train_by_genre |> 
  group_by(genres) |>
...
```{r}
fixed_countries <- lm(imdb_score ~ 0 + production_countries,
...
La separación la haremos utilizando la función `initial_split` del paquete *{tidy_models}*. Separamos dos tercios de la muestra para entrenamiento y un tercio para el testeo. También deifimos la función de pérdida con la que vamos a evaluar los modelos (`loss`) que es el error cuadrático medio.
...
El primer modelo que vamos a probar es un modelo linear de efectos mixtos con el año de estreno (`release_year`) y la duración de la película (`runtime`) como factores fijos y el director (`director`) de la cinta como factor aleatorio. El ajuste lo haremos utilizando la función `lmer` del paquete *{lme4}*.
...
error1 <- loss(pred_director$pred, test$imdb_score)
```

#### Modelo 2: Modelo GAM con país de producción como factor aleatorio.

El segundo modelo que vamos a probar es un modelo ajustando splines utilizando la función `gam` del paquete *{mgcv}*. En este caso al año de estreno (`release_year`), la duración de la película (`runtime`) y el país de producción (`production_countries`). Los dos primeros son factores fijos mientras que el país de producción es un factor aleatorio.  
...
Este modelo es muy similar al modelo dos con la diferencia de que en lugar de utilizar al país de producción de la película como factor aleatorio (modelado con splines cúbicas) como predictor, utilizamos al género de la película (`genres`).
...
Finalmente, comparamos el error cuadrático medio de los tres modelos AGREGAR REFERENCIA A TABLA Y NÚMERO. Podemos ver que el modelo con menor error es el Modelo 3 (splines cúbicas y género como factor aleatorio).  El error cuadrático medio es cercano a 1, lo que nos resulta razonable para la calificación de una película

```{r, echo=FALSE, warning=FALSE}
errores <- tibble(Modelo = c("Modelo lineal con director como FA",
                             "GAM con país como FA",
                             "GAM con género como FA"),
                  RMS = round(c(error1, error2, error3), 3))
...
Como vimos en la sección anterior, el modelo 3 es el que tiene mejor desepeño con los datos de entrenamiento. Es por eso que lo utilizaremos para generar las predicciones del los datos de testeo.
...
Y ahora generamos una predicción y la guardamos en un archivo `predicciones.csv`.
...
```{r, warning=FALSE}
test_completo |>
  mutate(y_hat = predict(fit_final, newdata = tibble(release_year, runtime, genres))) |> 
