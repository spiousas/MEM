...
for (i in 1:Nrep) {
  ...
  beta0_norm_desconocido[i] <- (beta0[i]-4)/ (summary(model)$sigma*sqrt(1/n+mean(x)^2/((n-1)*sd(x)^2)))
  beta1_norm_desconocido[i] <- (beta1[i]-(-2))/ (summary(model)$sigma / sqrt((n-1)*sd(x)^2))
}

coefs <- tibble(beta0, beta1, beta0_norm_conocido, beta1_norm_conocido,
                beta0_norm_desconocido, beta1_norm_desconocido)
